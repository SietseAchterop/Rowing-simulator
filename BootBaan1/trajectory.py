...
print("TIMES: ", totalsteps, stroketime)
...
def calc_pos(pos, blheight, bbaan, time):
    """ Determine all marker positions for this moment   """
    ...
    """ 
       ...
       find angle at shoulder in (shoulder, handle, elbow)_ triangle using cosine rule:
          c^2 = a^2 + b^2 -2ab.cos(C)
          cangle = math.acos( (a^2 + b^2 - c^2)/(2ab) )
       then deterime projection of elbow on handle-shoulder line
    """
    tmp = (uarml*uarml + sphererad*sphererad - larml*larml)/(2*uarml*sphererad)
    if tmp > 1.0:
        tmp = 1.0
    cangle = math.acos(tmp)
    # distance from shoulder to elcenter
    p = uarml * math.cos(cangle) 
    re = uarml * math.sin(cangle)

    # normal of circle of elbow (points towards handle)
    elnormal = normalize(p_0 - shoulderleft)
    # centre of circle
    elcenter = shoulderleft + p*elnormal
    ...
    # about 45 degrees  (optimal?)
    theta = 0.75*pi
    elb = elcenter + re*(n1*math.cos(theta) + n2*math.sin(theta))
    setPosition('msElbow', elb)
    """
    If deze[1] >= p_0[1]:
    ...
    """ blades horizontal
      ...
      Blade markers needed at all? We keep the blade horizontal with constraints.
    """
    ...
    """
    # cangle the same as on starboard
    ...
    elcenter = shoulderright + p*elnormal
...
def main():
    ...
    with open(trfile, mode='w') as report_file:
        ...
        # We assume the startstroke takes 194 steps   (get that number after a first try)
        report_writer.writerow([Hz, Hz, 194+totalsteps*nmbrstrokes, nmbrmarkers, "m", Hz, 1, 10])
        line = ["Frame#", "Time"]
        ...
        # first stroke to start the boat moving
        #   in first stroke go from tempo=1 to tempo=strokerate.
        time = 0
        curpos = 0
        pos = pos_in_parts(curpos)
        ...
        print('stroke ', pos, time)
        time += 1

        """
             wat is een stapje bij dit tempo in normalized stroke position?
             stroke is nu 1/3 van de cyclus tijd:  strtime
             volgende punt in de pos-curve is 1/strtime verder
             pos curve is van 0 tot 100
        """
        while time < (200-1):
            tempo = 1 + (strokerate-1)*time/200
            # time of a stroke at this tempo
            strtime = 60/tempo*strokepart*Hz
            # in which position we are in the stroke
            #   with a bit of tuning to end at 100 with curpos
            #     do this once after changing the strokerate
            # Hz = 100
            if Hz == 100:
                curpos += (100/strtime) * 100/154.3
            else:
                # Hz = 200
                curpos += (100/strtime) * 100/77.2
            ...
            for x in position:
                ...
                nextstring.append(f'{x[2]:.3f}')
            report_writer.writerow(nextstring)
            print('stroke ', pos, time, 'curpos: ', curpos, 'tempo ', tempo)
            time += 1
        ...
        for n in range(nmbrstrokes):
            ...
            for t in range(aantal, 1, -1):
                ...
                if t == 0:
                    ...
                    print(nextstring, "\n")
                report_writer.writerow(nextstring)
                # print('recover ', pos, time)
                time += 1
            b_end += 20

            #  Entry step 1
            pos = pos_in_parts(2)
            print('entry1 ', pos, time)
            # set position
            calc_pos(pos, blheight+0.065, bbaan, time)
            nextstring = [time, (time)/Hz]
            for x in position:
                nextstring.append(f'{x[0]:.3f}')
                nextstring.append(f'{x[1]:.3f}')
            ...
            # Entry step 2
            pos = pos_in_parts(2)
            print('entry2 ', pos, time)
            # set position
            calc_pos(pos, blheight+0.035, bbaan, time)
            nextstring = [time, (time)/Hz]
            for x in position:
                nextstring.append(f'{x[0]:.3f}')
                nextstring.append(f'{x[1]:.3f}')
        ...
        #bbaan.printToXML("Trajectory_new.osim")
